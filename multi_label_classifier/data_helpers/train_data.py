...
class TrainData(TrainDataBase):
    ...
    def trans_label_to_index(labels, label_to_index):
        ...
        :return:
        """
        label_len = len(label_to_index)
        labels_idx = np.zeros((len(labels), label_len))

        for i in range(len(labels)):
            for label in labels[i]:
                labels_idx[i, label_to_index[label]] = 1
        return labels_idx
    ...
    def gen_data(self):
        ...
        if os.path.exists(os.path.join(self._output_path, "train_data.pkl")) and \
            ...
            return np.array(train_data["inputs_idx"]), train_data["labels_idx"], label_to_index
        ...
        return np.array(inputs_idx), labels_idx, label_to_index
